/**
 * KaTeX markdown-it 插件
 * 源码来自 https://github.com/imzbf/md-editor-v3
 */
...
interface KatexOptions {
  katexRef: ShallowRef
  inlineDelimiters?: Array<{ open: string; close: string }>
  blockDelimiters?: Array<{ open: string; close: string }>
}
...
const create_math_block =
  ...
  (state, start, end, silent) => {
    ...
    let next = start // eslint-disable-line no-useless-assignment
...
const KatexPlugin = (
  ...
  const katexInline: RenderRule = (tokens, idx, _options, _env, slf) => {
  ...
  const katexBlock: RenderRule = (tokens, idx, _options, _env, slf) => {
    const token = tokens[idx]
    if (!token) {
      return ''
    }
